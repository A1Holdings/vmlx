...
- **Merge-queue serialization + cross-model diff review** before landing; inject per-worker `PORT`/service namespaces (worktrees alone leave port/DB collisions). The cross-model diff review is an **autonomous LLM reviewer**, not a human approval — it accepts/rejects/sends-back automatically. The merge queue is also where the **optional human-gate toggle** (below) inserts its `pending-approval` hold when enabled.
...
> **Autonomy principle (design constraint):** the system is **fully autonomous end to end by default** — no human-in-the-loop approval anywhere on the hot path (not on decomposition, not on egress, not on merges). All gates are *automated* policy/checks. A human can still inspect after the fact via the dashboard/audit log, but nothing waits for a person.

> **Optional human-gate toggle (opt-in, default OFF):** a single config switch (`humanApproval` / `--require-approval`, default `false`) that, when enabled, inserts one **pause-before-landing** checkpoint. It is purely additive and does **not** change default behavior:
> - **Default (`false`):** fully autonomous as above — the autonomous cross-model diff reviewer + `terminal-bench` gate land changes with no person involved.
> - **Enabled (`true`):** after all automated gates pass, the merge queue holds the change in a `pending-approval` state and waits for an explicit approve/reject (dashboard button, CLI `ao approve <session>`, or PR review) before landing on `main`. Optionally scope it (e.g., approve only when the diff touches protected paths, exceeds a size threshold, or fails the diff reviewer's confidence bar) so most changes still land autonomously and only risky ones pause.
> - A separate **global kill-switch** (`ao pause` / `ao stop`) can quiesce the fleet at any time regardless of the toggle: stop admitting new tasks, optionally cancel in-flight workers, and freeze the merge queue. This is operational control, not a hot-path gate.
...
- **Phase 2 — Orchestrator core.** Fork-rebase `agent-orchestrator`; repoint `decomposer.ts` + `Agent.getEnvironment()` to the gateway; **replace LLM-driven control flow with a deterministic scheduler + merge queue + autonomous cross-model diff review**; default to full autonomy (`humanApproval=false`) with the **opt-in toggle + global kill-switch** wired into the merge queue; keep AO's worktree/reaction/recovery/dashboard machinery. Enforce structured-JSON+git-commit signaling.
